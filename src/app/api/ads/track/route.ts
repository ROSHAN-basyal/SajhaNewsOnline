...
export async function GET(request: NextRequest) {
  try {
    ...
    analytics?.forEach(record => {
      const adId = record.advertisements.id
      const adTitle = record.advertisements.title
      const adPlacement = record.advertisements.placement
    ...
    Object.values(summary).forEach((ad: any) => {
      ad.ctr = ad.impressions > 0 ? (ad.clicks / ad.impressions * 100).toFixed(2) : '0.00'
    })
